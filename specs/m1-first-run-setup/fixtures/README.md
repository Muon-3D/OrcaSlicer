...
- The country lists are samples, not the certified list. The real list comes from Aux `/region/options`.
...
| `state.03-region-apply.json` | Connect was pressed with "Region: United Kingdom". `op.kind = region_apply`, and the hotspot will drop for about 8 s. |
| `state.04-joining.json` | The region is declared as GB (`source: ap`). `op.kind = join`, `phase = dhcp`. |
...
| `options.picker-eu.json` | A unit whose token offers European configurations, with default DE and English tier-2 countries. |
| `options.locked-us.json` | A US unit (token `configs: ["us"]`). There is no picker. |
| `options.none.json` | No valid token. The world domain `00` applies, and there is a support code. |
| `networks.eu-unit.json` | Scan results: a network on ch 13, an Enterprise network, an unsupported WEP network, and each suggestion source. |

...
| **S6 Link** | `remote.mode == "cloud"` and the step isn't done | Renders `remote.link` (muon-link's `LinkPhase`). **`connecting`:** "Getting a code…". **`code`:** the code in big mono digits, updating live as codes renew, with "Enter this code at **<host>** on any phone or computer that's online, then confirm on Walnut's screen." `<host>` is the host of `remote.link.url`, never a hard-coded console host (it changes at the KAN-414 cutover). A plain link opens `remote.link.url` (no `target`, because of the captive-portal window). **`offer`:** "Confirm on Walnut's screen" and the account. **`linked`:** ✓ "Linked to {account}". **`failed`:** the `link_failed` copy. | **Do this later** posts `remote: later`. |

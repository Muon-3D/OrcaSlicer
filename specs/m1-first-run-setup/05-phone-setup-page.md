...
| **S5 Finish** | cursor is `name`, `update` or `remote` | One page, as in mockup 6. **Name**: a text field pre-filled with `name.value`, max 32. **Update**, only if `update.status == pending`: "Update available · 1.4" with **Install now** / **Later**. **Remote access**: "Keep it on my network" (default) or "Link a Muon account", each with its one-line explanation, plus "My own server" if `capabilities.self_hosted`, which adds a URL field. | **Continue** posts `name`, then `update: later` if the update wasn't installed, then `remote`, in that order. Each post uses the `rev` returned by the one before. |
| **S5u Updating** | `op.kind == "update_install"` | Progress bar from `op.progress`, and "Walnut will restart. This page reconnects by itself." | – |
| **S6 Link** | `remote.mode == "cloud"` and the step isn't done | The current code in big mono digits (`482 913`), which updates live as codes renew. Text: "Enter this code at **app.muon3d.com** on any phone or computer that's online, then confirm on Walnut's screen." A button **Open app.muon3d.com** links to `https://app.muon3d.com/link?code=<code>` (plain link; `target` omitted for the captive-portal window). Also a **Do this later** button. After `phase: offer`: "Confirm on Walnut's screen". After `linked`: ✓ "Linked to jed@…". | Do this later posts `remote: later`. |

...
- **AP-7 and `10-ap-isolate`** say hotspot clients reach only the printer. **That isn't true today:** NetworkManager's shared mode NATs them out, `10-ap-isolate` only covers the `wlan0` subnet, and `ap0` also admits tcp/22, udp/5353 and udp/7127. OS-1 makes it true (S7).
...
| **Remote (Iroh) callers can write `/server/aux/wifi/*`** once FL-9 puts Fluidd's Aux client on Iroh: connect, forget, and the hotspot controls. S5 covers only `muon_setup`. | A remote Wi-Fi change can strand the printer offline, with no way back except the panel. | **Decision needed** (owner): refuse remote writes to `/server/aux/wifi/*` (reads stay allowed), or keep them and have the surfaces warn. Until decided, FL-9 must not add new remote write paths. |

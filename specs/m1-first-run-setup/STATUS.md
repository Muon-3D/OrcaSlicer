...
| OR-2 | OrcaSlicer | [Muon-3D/OrcaSlicer#4](https://github.com/Muon-3D/OrcaSlicer/pull/4) | In review | Matches the spec. It uses `/#/setup`, which works before OS-1's redirect lands. The Linux and macOS builds passed. The Windows build failed on a deps cache miss (`fail-on-cache-miss`) before any code compiled, and "Build all" is also red on `main`. Flatpak aarch64 failed too, while building the `orca_wxwidgets` dependency: 2 of 3 hunks of the `src/gtk/settings.cpp` patch don't apply. That's before any OrcaSlicer code is built, so it isn't caused by this PR. |
